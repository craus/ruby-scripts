...
class Expression
  ...
  def calc
    puts "e: #{self}" if $debug
    if @value != :undefined
      puts "#{self} = #{@value}" if $debug  
      return @value
    end
    result = case
      when function == 'nil' then nil
      when function == 'pair' then args.map(&:calc)
      when function == 'first' then args[0].calc[0] rescue raise LangException, "Called first(nil)", caller
      when function == 'second' then args[0].calc[1] rescue raise LangException, "Called second(nil)", caller
...
def calc_function name, args
  ...
  puts "#{name}(#{args.map{|x,y| "#{x} = #{y}"}.join ', '})" if $debug
  result = substitute(body, args).calc
  puts "#{name}(#{args.map{|x,y| "#{x} = #{y}"}.join ', '}) = #{result}" if $debug
  result
end
